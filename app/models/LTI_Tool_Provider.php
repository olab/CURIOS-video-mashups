...
### Set debug mode
#
    $this->debugMode = true;//isset($_POST['custom_debug']) && (strtolower($_POST['custom_debug']) == 'true');
#
...
#
    ...
    function lti_do_connect()
    {
        ...
        if(empty($role) || !in_array($role, ['superuser', 'author'])){//$this->consumer->role != $role
            $this->message = 'User has chosen wrong type of permission (the role).';
            return false;
        }

        $email        = !empty($this->user->email) ? $this->user->email : null;
        $consumer_key = $this->resource_link->getKey();

        $user = User::getByEmail($email);

        if(empty($user)){
            $user = User::getByEmail($consumer_key);
        }

        if(empty($user)){
            $user = new User;
            $user->created_by_lti = 1;
        }
